...
- [ ] T007 Create database config module at src/db/config.py with env-based connection string parsing and async engine setup
...
**Checkpoint**: Foundation ready - database provisioning, schema migrations, and SQLModel models are in place
...
- [ ] T012 [US1] Create migration 001_initial_schema.sql at src/db/migrations/001_initial_schema.sql with: CREATE EXTENSION vector, ingested_records table (UUID PK, source_type, locality, timestamp, ingestion_timestamp, source_url, point_of_origin, JSONB payload, VECTOR(1536) embedding), sources_registry table (UUID PK, name, source_type, locality, JSONB config, last_run, status, created_at, updated_at)
- [ ] T013 [US1] Add all required indexes to migration: idx_ingested_upsert_key (unique on source_url + point_of_origin), idx_ingested_source_type, idx_ingested_locality, idx_ingested_timestamp, idx_ingested_source_type_locality, idx_sources_type (unique), idx_sources_locality, idx_sources_status
...
## Phase 4: User Story 2 - SQLModel Entity Models (Priority: P2)
...
## Phase 5: User Story 3 - IngestionResult Validation (Priority: P3)
...
- [ ] T020 [US3] Create validation module at src/ingestion/validator.py with validate_ingestion_result() function that checks VR-001 through VR-007 rules (source_url non-empty valid URL, point_of_origin non-empty, payload.title/content_summary/raw_text non-empty, payload.metadata non-null, payload.related_resources non-null)
...
## Phase 6: User Story 4 - Verification and Integration Testing (Priority: P4)
...
- [ ] T025 [US4] Create contract test at tests/contract/test_ingestion_result.py that validates IngestionResult model enforces all VR-001 through VR-007 constraints via Pydantic validation
...
**Checkpoint**: All integration and unit tests pass against live PostgreSQL; IngestionResult contract enforced
...
- [ ] T028 Update docs/specifications/ folder to reflect current implementation (Single Source of Truth per constitution)
...
- **User Story 4 (P4)**: Depends on US1 + US2 + US3 - Integration tests require all components
...
- Once Foundational phase completes, User Stories 1-3 can proceed in parallel
...
3. Complete Phase 3: User Story 1 (PostgreSQL + Schema)
4. Complete Phase 4: User Story 2 (SQLModel models)
5. Complete Phase 5: User Story 3 (IngestionResult validation)
6. **STOP and VALIDATE**: Apply migrations, run psql verification queries, test validator
7. Deploy/demo if ready

### Incremental Delivery

1. Complete Setup + Foundational → Foundation ready
2. Add User Story 1 → Apply migrations, verify schema → Demo (PostgreSQL running with correct tables)
3. Add User Story 2 → Import and test SQLModel models → Demo (ORM layer working)
4. Add User Story 3 → Validate IngestionResult contract → Demo (validation pipeline working)
5. Add User Story 4 → Run integration tests → Demo (full pipeline verified)
...
2. Once Foundational is done:
   - Developer A: User Story 1 (PostgreSQL + Schema)
   - Developer B: User Story 2 (SQLModel models)
   - Developer C: User Story 3 (IngestionResult validation)
