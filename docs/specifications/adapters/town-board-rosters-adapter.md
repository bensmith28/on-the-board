...
[Guidance: Define the specific keys that will be populated within the `payload` column of the `ingested_records` table. Ensure these are consistent with the [Storage Specification](./docs/specifications/storage.md).]
...
### Error Handling
[Guidance: Describe how the adapter will handle failures (e.g., retry logic, alerting via the Core Engine).]
